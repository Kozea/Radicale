...
class Collection(BaseCollection):
    ...
    def serialize(self):
        ...
        if self.configuration.getboolean("logging", "performance"):
            self.logger.info("Collection read %d items in %.3f sec from %s", len(items),(time_end - time_begin).total_seconds(), self._filesystem_path)
        else:
            self.logger.debug("Collection read %d items in %.3f sec from %s", len(items),(time_end - time_begin).total_seconds(), self._filesystem_path)
        result = ""
